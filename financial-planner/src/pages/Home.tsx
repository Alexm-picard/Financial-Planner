...
const Home: React.FC = () => {
  ...
  const handleAddAccount = async () => {
    ...
    try {
      let finalBalance = newAccount.balance;
      
      if (isCreatingAccountForSweep && selectedAccount) {
        finalBalance = newAccount.balance + selectedAccount.balance;
      }
      ...
      if (accountId) {
        await addTransaction({
          ...
          userId: user.uid,
          type: 'create',
          newBalance: newAccount.type === 'debt' ? -Math.abs(finalBalance) : Math.abs(finalBalance),
    ...
    <Box>
      ...
      {accountsLoading ? (
        ...
        <EmptyState
          ...
          description="Get started by adding your first account to track your finances."
          action={{
            label: 'Add Account',
        ...
        <>
          <AccountSection
            ...
            subtitle="Your savings accounts and assets"
            accounts={savingsAccounts}
            onEdit={(acc) => {
              setSelectedAccount({ ...acc });
              setIsEditDialogOpen(true);
            }}
            emptyIcon={<AccountBalanceIcon sx={{ fontSize: 48 }} />}
            emptyTitle="No Savings Accounts"
            emptyDescription="Add a savings account to start tracking your assets."
          />
          ...
          <AccountSection
            ...
            emptyIcon={<CreditCardIcon sx={{ fontSize: 48 }} />}
            emptyTitle="No Debt Accounts"
            emptyDescription="Add a debt account to track your liabilities."
          />
        </>
      )}
